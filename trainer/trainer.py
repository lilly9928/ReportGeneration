...
from base.MRG.utils.load_data import get_loader
from models.model_CNN_Decoder import Impression_Decoder,EncoderCNN,Atten_Sen_Decoder
from metrics import compute_metrics, generate_text_file
from utils.logger import create_logger
from base.MRG.IUdata.build_vocab import JsonReader, Vocabulary
...
def train_net(num_run, logger, args):
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.log_metrics_step == 0:
            pre_imps_lst, pre_fins_lst, img_id_lst = train(image_encoder, impression_decoder,
                                                                       finding_decoder, vocab, args)
            imp_fin_bleu, imp_fin_rouge, imp_fin_cider = compute_metrics(pre_imps_lst, pre_fins_lst,
                                                                                         img_id_lst, args)
...
def train(image_encoder, impression_decoder, finding_decoder, vocab, args):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_dir', type=str, default='D:/data/iuct/preprocess/NLMCXR_Frontal', help='directory for X-ray images')
