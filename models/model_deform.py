...
class DAT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, expansion=4,
        ...
        # LAYERS
        self.positional_encoder = PositionalEncoding(dim_model=dims[3], dropout_p=0.01, max_len=5000)
        self.embedding = nn.Embedding(num_tokens, dims[3])
        decoder_layer = nn.TransformerDecoderLayer(d_model=dims[3], nhead=num_heads)
        self.TransformerDecoder = nn.TransformerDecoder(decoder_layer, num_layers=num_decoder_layers)
        self.out = nn.Linear(dims[3], num_tokens)
    ...
    def forward(self, img, caption,tgt_mask):
        ...
        return outputs, positions, references


    def example_images(self,oimg, img, start_token, vocabulary, device, max_length=50, dim_model=64):
        result_caption = []
